...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const subscription of subscriptions as unknown as SubscriptionWithDetails[]) {
      ...
      // Apply keyword filters if they exist
      ...
      // Apply spse filters if they exist
      if (spse && spse.length > 0) {
        const spseFilters = spse
          .map((site) => `source_url.ilike.%//spse.inaproc.id/${site}%`)
          .join(",");
        filterConditions.push(`(${spseFilters})`);
      }

      // Apply the status filter (this should always be present)
      ...
      // Apply all collected conditions using a single .or() method
